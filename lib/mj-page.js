...
function GetWindow() {
  document = jsdom('', {
    // userAgent: 'Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/53.0.2785.122 Safari/537.36 Vivaldi/1.4.589.29'
    ...
    // userAgent: 'Prince/7.1 (http://www.princexml.com)'
  });
...
function ConfigureMathJax() {
  window.MathJax = {
    ...
    //
    jax: ["input/TeX", "input/MathML", "input/AsciiMath", "output/SVG", "output/NativeMML", "output/CommonHTML", "output/HTML-CSS"],
    ...
    'HTML-CSS': {EqnChunk: 1000000, EqnDelay: 0, undefinedFamily:"monospace"},
    ...
    AuthorInit: function () {
      ...
      MathJax.Message.Set = function (text,n,delay) {
        if (displayMessages && n !== 0) {
          ...
          if (text[1].indexOf('web-font') >= 0) {
            console.trace('1');
          }
          if (text instanceof window.Array)
            {text = MathJax.Localization._.apply(MathJax.Localization,text)}
          console.log('\033[94m[MathJax debug]\033[0m', text);
        }/***/
      };
      ...
      MathJax.Hub.Register.StartupHook("HTML-CSS Jax Config",function () {
        var HTMLCSS = MathJax.OutputJax['HTML-CSS'], HTML = MathJax.HTML;

        //
        //  Don't need these styles
        //
        var STYLES = HTMLCSS.config.styles;
        /*
        .MathJax_Display
        .MathJax .merror
        ...
        HTMLCSS.Augment({
          ...
          getDefaultExEm: function () {
            var styles = document.head.getElementsByTagName("style");
            HTMLCSSSTYLES = styles[styles.length-1].innerHTML;
            this.pxPerInch    = 96;
            this.defaultEx    = 6;
            this.defaultEm    = 6 / HTMLCSS.TeX.x_height * 1000;
            this.defaultWidth = 100;
          },
          //
          //  Adjust preTranslate() to not try to find the ex-size or
          ...
          preTranslate: function (state) {
            ...
            for (i = 0; i < m; i++) {
              ...
              node = HTMLCSS.Element("mjx-chtml",{
                id:jax.inputID+"-Frame", isMathJax:true, jaxID:this.id
              });
              if (jax.HTMLCSS.display) {
                ...
                // Zoom box requires an outer container to get the positioning right.
                //
                var NODE = HTMLCSS.Element("mjx-chtml",{className:"MJXc-display",isMathJax:false});
                NODE.appendChild(node); node = NODE;
              }
              //
              //  Mark math for screen readers
              ...
              script.parentNode.insertBefore(node,script);
              //
              //  Set HTMLCSS data for jax
              //
              jax.HTMLCSS.ex = ex = (data||defaults).ex;
              jax.HTMLCSS.em = em = ex / HTMLCSS.TeX.x_height; // scale ex to x_height
              jax.HTMLCSS.cwidth = width / em;
              jax.HTMLCSS.lineWidth = (linebreak ? width / em : HTMLCSS.BIGDIMEN);
              jax.HTMLCSS.scale = 1; jax.HTMLCSS.fontsize = "100%";
            }
            //
            //  Set state variables used for displaying equations in chunks
            //
...
function ConfigureTypeset() {
  ...
  CHTML.defaultEx                   = SVG.defaultEx                  ;
  CHTML.defaultWidth                = SVG.defaultWidth               ;
  CHTML.config.linebreaks.automatic = SVG.config.linebreaks.automatic;
  CHTML.config.linebreaks.width     = SVG.config.linebreaks.width    ;

  HTMLCSS.defaultEx                   = SVG.defaultEx                  ;
  HTMLCSS.defaultWidth                = SVG.defaultWidth               ;
  HTMLCSS.config.linebreaks.automatic = SVG.config.linebreaks.automatic;
  HTMLCSS.config.linebreaks.width     = SVG.config.linebreaks.width    ;
...
function AdjustHTML() {
  ...
  if (data.renderer === "CommonHTML") {
    ...
    content.insertBefore(styles,content.firstChild);
  } else if (data.renderer === 'HTML-CSS') {
    //
    //  Add styles
    //
    var styles = document.createElement("style");
    styles.innerHTML = HTMLCSSSTYLES;
    styles.id="MathJax_HTMLCSS_styles";
    content.insertBefore(styles,content.firstChild);
  }
  return callback;
}
